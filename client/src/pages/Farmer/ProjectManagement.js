...
const ProjectManagement = () => {
  ...
  const { data: project, isLoading: projectLoading, error: projectError } = useQuery(
    ...
    },
    {
      ...
      onError: (error) => {
        ...
        toast.error(error.response?.data?.message || 'Erreur lors du chargement du projet');
      }
    }
  );
  ...
  const requestWithdrawalMutation = useMutation(
    async () => {
      return api.post(`/farmer/projects/${id}/request-withdrawal`);
    },
    {
      onSuccess: () => {
  ...
  const createUpdateMutation = useMutation(
    async (data) => {
      return api.post(endpoints.projects.addUpdate(id), data);
    },
    {
      onSuccess: () => {
  ...
  const editUpdateMutation = useMutation(
    async ({ updateId, data }) => {
      return api.put(`/farmer/projects/${id}/updates/${updateId}`, data);
    },
    {
      onSuccess: () => {
  ...
  const deleteUpdateMutation = useMutation(
    async (updateId) => {
      return api.delete(`/farmer/projects/${id}/updates/${updateId}`);
    },
    {
      onSuccess: () => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">{t('farmer.projectManagement.loading')}</p>
        </div>
      </div>
    );
  }
  ...
  if (projectError || (!projectLoading && !project)) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">{t('farmer.projectManagement.notFound')}</h2>
          <p className="text-gray-600 mb-4">
            {projectError?.response?.data?.message || t('farmer.projectManagement.noPermission')}
          </p>
          <button
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white rounded-xl shadow-sm p-6">
                <p className="text-sm text-gray-500 mb-2">{t('farmer.projectManagement.budget')}</p>
                <p className="text-2xl font-bold text-emerald-600">
                  {parseFloat(project.budget_gyt).toLocaleString()} DOLLAR
                </p>
              </div>

              <div className="bg-white rounded-xl shadow-sm p-6">
                <p className="text-sm text-gray-500 mb-2">{t('farmer.projectManagement.funded')}</p>
                <p className="text-2xl font-bold text-blue-600">
                  {parseFloat(project.funded_amount_gyt).toLocaleString()} DOLLAR
                </p>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6">
                <p className="text-sm text-gray-500 mb-2">{t('farmer.projectManagement.investors')}</p>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-sm text-gray-500">{t('farmer.projectManagement.estimatedReturn')}</p>
                ...
                <div>
                  <p className="text-sm text-gray-500">{t('farmer.projectManagement.status')}</p>
        ...
        {activeTab === 'updates' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-bold">{t('farmer.projectManagement.updatesTitle')}</h2>
              <button
            ...
            {updatesLoading ? (
              ...
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">{t('farmer.projectManagement.noUpdates')}</h3>
        ...
        {activeTab === 'withdrawal' && (
          <div className="space-y-6">
            <h2 className="text-2xl font-bold">{t('farmer.projectManagement.withdrawal')}</h2>
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <p className="font-medium">
                            {parseFloat(request.amount_gyt).toLocaleString()} DOLLAR
                          </p>
