...
const Notifications = () => {
  ...
  const markAllMutation = useMutation(async () => {
    ...
    onSuccess: () => { toast.success('Toutes les notifications ont été marquées comme lues'); qc.invalidateQueries(['notifications-page']); qc.invalidateQueries(['notifications']); },
    onError: (e) => toast.error(e?.response?.data?.message || 'Échec de l\'opération')
  });
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-600">Non lues: {unreadCount}</span>
            <button className="btn btn-outline" onClick={() => markAllMutation.mutate()} disabled={markAllMutation.isLoading || unreadCount === 0}>Tout marquer comme lu</button>
            <BackButton />
          </div>
        </div>
          ...
          {notifications.map(n => (
            <div key={n.id} className={`p-3 border rounded flex items-start justify-between ${n.is_read ? 'bg-white' : 'bg-gray-50 border-primary-200'}`}>
              <div className="flex items-start gap-3 pr-3">
                ...
                <div>
                  <div className={`font-medium ${n.is_read ? '' : 'text-gray-900'}`}>{n.title || 'Notification'}</div>
                  <div className="text-sm text-gray-700">{n.message}</div>
                  <div className="text-xs text-gray-500 mt-1">{new Date(n.created_at).toLocaleString()}</div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {!n.is_read && <button className="btn btn-outline btn-sm" onClick={() => markRead(n.id)}>Marquer comme lue</button>}
              </div>
            </div>
          ))}
          {notifications.length === 0 && !isLoading && (
            <div className="card text-gray-600">Aucune notification.</div>
          )}
        </div>
      </div>
    </div>
